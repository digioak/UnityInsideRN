...
namespace HutongGames.PlayMaker.Actions
{
    ...
    public class DOTweenControlMethodsPlayById : FsmStateAction
    {
        ...
        public override void OnEnter()
        {
            ...
            switch (tweenIdType)
            {
                case Doozy.PlayMaker.Actions.TweenId.UseString: if (string.IsNullOrEmpty(stringAsId.Value) == false) numberOfTweensPlayed = DOTween.Play(stringAsId.Value); break;
                case Doozy.PlayMaker.Actions.TweenId.UseTag: if (string.IsNullOrEmpty(tagAsId.Value) == false) numberOfTweensPlayed = DOTween.Play(tagAsId.Value); break;
                case Doozy.PlayMaker.Actions.TweenId.UseGameObject: if (gameObjectAsId.Value != null) numberOfTweensPlayed = DOTween.Play(gameObjectAsId.Value); break;
            }
            if (debugThis.Value) State.Debug("DOTween Control Methods Play By Id - Played " + numberOfTweensPlayed + " tweens");
            Finish();
        }
    }
}

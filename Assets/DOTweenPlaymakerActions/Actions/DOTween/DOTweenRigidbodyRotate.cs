...
namespace HutongGames.PlayMaker.Actions
{
    ...
    public class DOTweenRigidbodyRotate : FsmStateAction
    {
        [RequiredField]
        ...
        public override void OnEnter()
        {
            switch (target)
            {
                case Doozy.PlayMaker.Actions.Target.Value:
                    tween = Fsm.GetOwnerDefaultTarget(gameObject).GetComponent<Rigidbody>().DORotate(to.Value, duration.Value, rotateMode);
                    break;
                case Doozy.PlayMaker.Actions.Target.GameObject:
                    tween = Fsm.GetOwnerDefaultTarget(gameObject).GetComponent<Rigidbody>().DORotate(toGameObject.Value.transform.rotation.eulerAngles, duration.Value, rotateMode);
                    break;
            }
            ...
            tween.Play();
            if (debugThis.Value) State.Debug("DOTween RigidBody Rotate");

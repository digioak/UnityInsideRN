...
namespace HutongGames.PlayMaker.Actions
{
    ...
    public class DOTweenRigidbodyMove : FsmStateAction
    {
        [RequiredField]
        ...
        public override void OnEnter()
        {
            switch (target)
            {
                case Doozy.PlayMaker.Actions.Target.Value:
                    tween = Fsm.GetOwnerDefaultTarget(gameObject).GetComponent<Rigidbody>().DOMove(to.Value, duration.Value, snapping.Value);
                    break;
                case Doozy.PlayMaker.Actions.Target.GameObject:
                    tween = Fsm.GetOwnerDefaultTarget(gameObject).GetComponent<Rigidbody>().DOMove(toGameObject.Value.transform.position, duration.Value, snapping.Value);
                    break;
            }
            ...
            tween.Play();
            if (debugThis.Value) State.Debug("DOTween RigidBody Move");

...
namespace HutongGames.PlayMaker.Actions
{
    ...
    [Tooltip("Punches a Transform's localPosition towards the given direction and then back to the starting one as if it was connected to the starting position via an elastic. ")]
    ...
    public class DOTweenTransformPunchPosition : FsmStateAction
    {
        ...
        {
            tween = Fsm.GetOwnerDefaultTarget(gameObject).GetComponent<Transform>().DOPunchPosition(punch.Value, duration.Value, vibrato.Value, elasticity.Value, snapping.Value);
            ...
            tween.Play();
            if (debugThis.Value) State.Debug("DOTween Transform Punch Position");

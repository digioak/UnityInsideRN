...
namespace HutongGames.PlayMaker.Actions
{
    ...
    public class DOTweenTransformMoveX : FsmStateAction
    {
        ...
        [ActionSection("")]
        [Tooltip("Select the target. Either a Vector3 or a GameObject's x position")]
        ...
        public override void OnEnter()
        {
            switch (target)
            {
                case Doozy.PlayMaker.Actions.Target.Value:
                    tween = Fsm.GetOwnerDefaultTarget(gameObject).GetComponent<Transform>().DOMoveX(to.Value, duration.Value, snapping.Value);
                    break;
                case Doozy.PlayMaker.Actions.Target.GameObject:
                    tween = Fsm.GetOwnerDefaultTarget(gameObject).GetComponent<Transform>().DOMoveX(toGameObject.Value.transform.position.x, duration.Value, snapping.Value);
                    break;
            }
            ...
            tween.Play();
            if (debugThis.Value) State.Debug("DOTween Transform Move X");

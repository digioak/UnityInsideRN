...
namespace HutongGames.PlayMaker.Actions
{
    ...
    [Tooltip("Tweens the target's color to the given value, in a way that allows other DOBlendableColor tweens to work together on the same target, instead than fight each other as multiple DOColor would do.")]
    [HelpUrl("http://dotween.demigiant.com/documentation.php")]
    public class DOTweenLightBlendableColor : FsmStateAction
    {
        [RequiredField]
        ...
        {
            tween = Fsm.GetOwnerDefaultTarget(gameObject).GetComponent<Light>().DOBlendableColor(to.Value, duration.Value);
            ...
            tween.Play();
            if (debugThis.Value) State.Debug("DOTween Light Blendable Color");

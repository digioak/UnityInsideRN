...
namespace HutongGames.PlayMaker.Actions
{
    ...
    [Tooltip("Moves the target's localPosition to the given value, tweening only the Z axis. ")]
    [HelpUrl("http://dotween.demigiant.com/documentation.php")]
    public class DOTweenTransformLocalMoveZ : FsmStateAction
    {
        [RequiredField]
        ...
        [ActionSection("")]
        [Tooltip("Select the target. Either a float or a GameObject's z position")]
        ...
        public override void OnEnter()
        {
            switch (target)
            {
                case Doozy.PlayMaker.Actions.Target.Value:
                    tween = Fsm.GetOwnerDefaultTarget(gameObject).GetComponent<Transform>().DOLocalMoveZ(to.Value, duration.Value, snapping.Value);
                    break;
                case Doozy.PlayMaker.Actions.Target.GameObject:
                    tween = Fsm.GetOwnerDefaultTarget(gameObject).GetComponent<Transform>().DOLocalMoveZ(toGameObject.Value.transform.position.z, duration.Value, snapping.Value);
                    break;
            }
            ...
            tween.Play();
            if (debugThis.Value) State.Debug("DOTween Transform Local Move Z");

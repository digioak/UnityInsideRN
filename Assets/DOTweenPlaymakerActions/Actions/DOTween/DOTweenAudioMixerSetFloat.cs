...
namespace HutongGames.PlayMaker.Actions
{
    ...
    [Tooltip("Tweens an AudioMixer's exposed float to the given value. Note that you need to manually expose a float in an AudioMixerGroup in order to be able to tween it from an AudioMixer.")]
    [HelpUrl("http://dotween.demigiant.com/documentation.php")]
    public class DOTweenAudioMixerSetFloat : FsmStateAction
    {
        [RequiredField] public FsmOwnerDefault gameObject;

        [UIHint(UIHint.Description)] public string descriptionArea = "Drag and Drop an AudioMixer from the ProjectView";

        [RequiredField] [Tooltip("The AudioMixer you want to control")]
        public AudioMixer audioMixer;

        [RequiredField] [UIHint(UIHint.FsmFloat)] [Tooltip("Exposed parameter name of type float")]
        public FsmString floatName;

        [RequiredField] [UIHint(UIHint.FsmFloat)] [Tooltip("The end value to reach")]
        public FsmFloat to;
        ...
        [RequiredField] [UIHint(UIHint.FsmFloat)] [Tooltip("The duration of the tween")]
        public FsmFloat duration;

        [UIHint(UIHint.FsmBool)] [Tooltip("If isSpeedBased is TRUE sets the tween as speed based (the duration will represent the number of units/degrees the tween moves x second). NOTE: if you want your speed to be constant, also set the ease to Ease.Linear.")]
        public FsmBool setSpeedBased;

        [UIHint(UIHint.FsmFloat)] [Tooltip("Set a delayed startup for the tween")]
        public FsmFloat startDelay;

        [ActionSection("Reverse Options")] [UIHint(UIHint.FsmBool)] [Tooltip("Play in reverse. Changes a TO tween into a FROM tween: sets the current target's startValue as the tween's endValue then immediately sends the target to the previously set endValue.")]
        public FsmBool playInReverse;

        [UIHint(UIHint.FsmBool)] [Tooltip("If TRUE the FROM value will be calculated as relative to the current one")]
        public FsmBool setReverseRelative;

        [ActionSection("Events")] [UIHint(UIHint.FsmEvent)]
        public FsmEvent startEvent;

        [UIHint(UIHint.FsmEvent)] public FsmEvent finishEvent;

        [UIHint(UIHint.FsmBool)] [Tooltip("If TRUE this action will finish immediately, if FALSE it will finish when the tween is complete.")]
        public FsmBool finishImmediately;

        [ActionSection("Tween ID")] [UIHint(UIHint.Description)]
        ...
        [UIHint(UIHint.FsmString)] [Tooltip("Use a String as the tween ID")]
        public FsmString stringAsId;

        [UIHint(UIHint.Tag)] [Tooltip("Use a Tag as the tween ID")]
        public FsmString tagAsId;

        [ActionSection("Ease Settings")] public Doozy.PlayMaker.Actions.SelectedEase selectedEase;
        ...
        [ActionSection("Loop Settings")] [UIHint(UIHint.Description)]
        public string loopsDescriptionArea = "Setting loops to -1 will make the tween loop infinitely.";

        [UIHint(UIHint.FsmInt)] [Tooltip("Setting loops to -1 will make the tween loop infinitely.")]
        public FsmInt loops;
        ...
        [ActionSection("Special Settings")] [UIHint(UIHint.FsmBool)] [Tooltip("If autoKillOnCompletion is set to TRUE the tween will be killed as soon as it completes, otherwise it will stay in memory and you'll be able to reuse it.")]
        public FsmBool autoKillOnCompletion;

        [UIHint(UIHint.FsmBool)] [Tooltip("Sets the recycling behaviour for the tween. If you don't set it then the default value (set either via DOTween.Init or DOTween.defaultRecyclable) will be used.")]
        ...
        [UIHint(UIHint.FsmBool)] [Tooltip("If TRUE the tween will ignore Unity's Time.timeScale. NOTE: independentUpdate works also with UpdateType.Fixed but is not recommended in that case (because at timeScale 0 FixedUpdate won't run).")]
        public FsmBool isIndependentUpdate;

        [ActionSection("Debug Options")] [UIHint(UIHint.FsmBool)]
        ...
        {
            base.Reset();
            floatName = new FsmString {UseVariable = false};
        ...
        public override void OnEnter()
        {
            if (audioMixer == null)
            {
                if (debugThis.Value) State.Debug("ERROR - DOTween AudioMixer SetFloat - There is no AudioMixer linked to this state");
                return;
            }

            tween = audioMixer.DOSetFloat(floatName.Value, to.Value, duration.Value);
            ...
            tween.Play();
            if (debugThis.Value) State.Debug("DOTween AudioMixer SetFloat");

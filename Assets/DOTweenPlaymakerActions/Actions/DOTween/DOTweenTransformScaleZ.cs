...
namespace HutongGames.PlayMaker.Actions
{
    ...
    [Tooltip("Scales the target's localScale to the given value while tweening only the Z axis.")]
    [HelpUrl("http://dotween.demigiant.com/documentation.php")]
    public class DOTweenTransformScaleZ : FsmStateAction
    {
        [RequiredField]
        ...
        [ActionSection("")]
        [Tooltip("Select the target. Either a float or a GameObject's z scale")]
        ...
        public override void OnEnter()
        {
            switch (target)
            {
                case Doozy.PlayMaker.Actions.Target.Value:
                    tween = Fsm.GetOwnerDefaultTarget(gameObject).GetComponent<Transform>().DOScaleZ(to.Value, duration.Value);
                    break;
                case Doozy.PlayMaker.Actions.Target.GameObject:
                    tween = Fsm.GetOwnerDefaultTarget(gameObject).GetComponent<Transform>().DOScaleZ(toGameObject.Value.transform.localScale.z, duration.Value);
                    break;
            }
            ...
            tween.Play();
            if (debugThis.Value) State.Debug("DOTween Transform Scale Z");

...
namespace HutongGames.PlayMaker.Actions
{
    ...
    [Tooltip("Rotates the target to the given value. Requires a Vector3 end value, not a Quaternion (if you really want to pass a Quaternion, just convert it using myQuaternion.eulerAngles).")]
    [HelpUrl("http://dotween.demigiant.com/documentation.php")]
    public class DOTweenTransformRotate : FsmStateAction
    {
        [RequiredField]
        ...
        [ActionSection("")]
        [Tooltip("Select the target. Either a Vector3 or a GameObject's rotation")]
        ...
        public override void OnEnter()
        {
            switch (target)
            {
                case Doozy.PlayMaker.Actions.Target.Value:
                    tween = Fsm.GetOwnerDefaultTarget(gameObject).GetComponent<Transform>().DORotate(to.Value, duration.Value, rotateMode);
                    break;
                case Doozy.PlayMaker.Actions.Target.GameObject:
                    tween = Fsm.GetOwnerDefaultTarget(gameObject).GetComponent<Transform>().DORotate(toGameObject.Value.transform.rotation.eulerAngles, duration.Value, rotateMode);
                    break;
            }
            ...
            tween.Play();
            if (debugThis.Value) State.Debug("DOTween Transform Rotate");

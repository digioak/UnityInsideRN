...
namespace HutongGames.PlayMaker.Actions
{
    ...
    [Tooltip("Scales the target's localScale to the given value. Passing a float instead of a Vector3 allows to scale stuff uniformly.")]
    ...
    public class DOTweenTransformScale : FsmStateAction
    {
        ...
        [ActionSection("")]
        [Tooltip("Select the target. Either a Vector3 or a GameObject's scale")]
        ...
        public override void OnEnter()
        {
            switch (target)
            {
                case Doozy.PlayMaker.Actions.Target.Value:
                    tween = Fsm.GetOwnerDefaultTarget(gameObject).GetComponent<Transform>().DOScale(to.Value, duration.Value);
                    break;
                case Doozy.PlayMaker.Actions.Target.GameObject:
                    tween = Fsm.GetOwnerDefaultTarget(gameObject).GetComponent<Transform>().DOScale(toGameObject.Value.transform.localScale, duration.Value);
                    break;
            }
            ...
            tween.Play();
            if (debugThis.Value) State.Debug("DOTween Transform Scale");

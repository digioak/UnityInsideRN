...
namespace HutongGames.PlayMaker.Actions
{
    ...
    [Tooltip("Rotates the target's localRotation to the given value. Requires a Vector3 end value, not a Quaternion (if you really want to pass a Quaternion, just convert it using myQuaternion.eulerAngles).")]
        ...
        public override void OnEnter()
        {
            switch (target)
            {
                case Doozy.PlayMaker.Actions.Target.Value:
                    tween = Fsm.GetOwnerDefaultTarget(gameObject).GetComponent<Transform>().DOLocalRotate(to.Value, duration.Value, rotateMode);
                    break;
                case Doozy.PlayMaker.Actions.Target.GameObject:
                    tween = Fsm.GetOwnerDefaultTarget(gameObject).GetComponent<Transform>().DOLocalRotate(toGameObject.Value.transform.rotation.eulerAngles, duration.Value, rotateMode);
                    break;
            }
            ...
            tween.Play();
            if (debugThis.Value) State.Debug("DOTween Transform Local Rotate");

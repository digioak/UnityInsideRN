...
namespace HutongGames.PlayMaker.Actions
{
    ...
    [Tooltip("Tweens a Transform's position BY the given value (as if it was set to relative), in a way that allows other DOBlendableMove tweens to work together on the same target, instead than fight each other as multiple DOMove would do.")]
    ...
    public class DOTweenTransformBlendableMoveBy : FsmStateAction
    {
        ...
        {
            tween = Fsm.GetOwnerDefaultTarget(gameObject).GetComponent<Transform>().DOBlendableMoveBy(by.Value, duration.Value, snapping.Value);
            ...
            tween.Play();
            if (debugThis.Value) State.Debug("DOTween Transform Blendable Move By");

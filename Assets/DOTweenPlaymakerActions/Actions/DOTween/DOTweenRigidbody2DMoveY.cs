...
namespace HutongGames.PlayMaker.Actions
{
    ...
    public class DOTweenRigidbody2DMoveY : FsmStateAction
    {
        [RequiredField]
        ...
        public override void OnEnter()
        {
            switch (target)
            {
                case Doozy.PlayMaker.Actions.Target.Value:
                    tween = Fsm.GetOwnerDefaultTarget(gameObject).GetComponent<Rigidbody2D>().DOMoveY(to.Value, duration.Value, snapping.Value);
                    break;
                case Doozy.PlayMaker.Actions.Target.GameObject:
                    tween = Fsm.GetOwnerDefaultTarget(gameObject).GetComponent<Rigidbody2D>().DOMoveY(toGameObject.Value.transform.position.y, duration.Value, snapping.Value);
                    break;
            }
            ...
            tween.Play();
            if (debugThis.Value) State.Debug("DOTween RigidBody2D Move Y");

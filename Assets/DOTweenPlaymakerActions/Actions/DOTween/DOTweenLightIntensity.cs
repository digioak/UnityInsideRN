...
namespace HutongGames.PlayMaker.Actions
{
    ...
    [Tooltip("Changes the light's intensity to the given one")]
    [HelpUrl("http://dotween.demigiant.com/documentation.php")]
    public class DOTweenLightIntensity : FsmStateAction
    {
        [RequiredField]
        ...
        {
            tween = Fsm.GetOwnerDefaultTarget(gameObject).GetComponent<Light>().DOIntensity(to.Value, duration.Value);
            ...
            tween.Play();
            if (debugThis.Value) State.Debug("DOTween Light Intensity");

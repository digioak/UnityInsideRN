...
namespace HutongGames.PlayMaker.Actions
{
    ...
    [Tooltip("Moves the target's position to the given value, tweening only the X axis.")]
    [HelpUrl("http://dotween.demigiant.com/documentation.php")]
    public class DOTweenRigidbody2DMoveX : FsmStateAction
    {
        [RequiredField]
        ...
        public override void OnEnter()
        {
            switch (target)
            {
                case Doozy.PlayMaker.Actions.Target.Value:
                    tween = Fsm.GetOwnerDefaultTarget(gameObject).GetComponent<Rigidbody2D>().DOMoveX(to.Value, duration.Value, snapping.Value);
                    break;
                case Doozy.PlayMaker.Actions.Target.GameObject:
                    tween = Fsm.GetOwnerDefaultTarget(gameObject).GetComponent<Rigidbody2D>().DOMoveX(toGameObject.Value.transform.position.x, duration.Value, snapping.Value);
                    break;
            }
            ...
            tween.Play();
            if (debugThis.Value) State.Debug("DOTween RigidBody2D Move X");

...
namespace HutongGames.PlayMaker.Actions
{
    ...
    [Tooltip("Punches the target's anchoredPosition with the given values. ")]
    ...
    public class DOTweenRectTransformPunchAnchorPos : FsmStateAction
    {
        ...
        [Tooltip("The direction and strength of the punch (added to the RectTransform's current position)")]
        ...
        public override void Reset()
        {
            base.Reset();
            gameObject = null;
            punch = new FsmVector2 { UseVariable = false };
        ...
        {
            tween = Fsm.GetOwnerDefaultTarget(gameObject).GetComponent<RectTransform>().DOPunchAnchorPos(punch.Value, duration.Value, vibrato.Value, elasticity.Value, snapping.Value);
            ...
            tween.Play();
            if (debugThis.Value) State.Debug("DOTween RectTransform Punch Anchor Pos");

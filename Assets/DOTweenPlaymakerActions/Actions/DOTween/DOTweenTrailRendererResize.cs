...
namespace HutongGames.PlayMaker.Actions
{
    ...
    [Tooltip("Tweens a TrailRenderer's startWidth/endWidth to the given value. Also stores the TrailRenderer as the tween's target so it can be used for filtered operations")]
    ...
    public class DOTweenTrailRendererResize : FsmStateAction
    {
        ...
        [Tooltip("The end startWidth to reach")]
        public FsmFloat toStartWidth;

        [RequiredField]
        [UIHint(UIHint.FsmFloat)]
        [Tooltip("The end endWidth to reach")]
        ...
        public override void Reset()
        {
            base.Reset();
            gameObject = null;
            toStartWidth = new FsmFloat { UseVariable = false };
            toEndWidth = new FsmFloat { UseVariable = false };
        ...
        {
            tween = Fsm.GetOwnerDefaultTarget(gameObject).GetComponent<TrailRenderer>().DOResize(toStartWidth.Value, toEndWidth.Value, duration.Value);
            ...
            tween.Play();
            if (debugThis.Value) State.Debug("DOTween Trail Renderer Resize");

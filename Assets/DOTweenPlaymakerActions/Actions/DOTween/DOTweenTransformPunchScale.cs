...
namespace HutongGames.PlayMaker.Actions
{
    ...
    [Tooltip("Punches a Transform's localScale towards the given size and then back to the starting one as if it was connected to the starting size via an elastic. ")]
    ...
    public class DOTweenTransformPunchScale : FsmStateAction
    {
        ...
        [Tooltip("The direction and strength of the punch (added to the Transform's current scale)")]
        ...
        {
            tween = Fsm.GetOwnerDefaultTarget(gameObject).GetComponent<Transform>().DOPunchScale(punch.Value, duration.Value, vibrato.Value, elasticity.Value);
            ...
            tween.Play();
            if (debugThis.Value) State.Debug("DOTween Transform Punch Scale");

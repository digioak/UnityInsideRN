...
namespace HutongGames.PlayMaker.Actions
{
    ...
    [Tooltip("Changes the layoutElement's preferredWidth/Height to the given one ")]
    ...
    public class DOTweenLayoutElementPreferredSize : FsmStateAction
    {
        ...
        {
            tween = Fsm.GetOwnerDefaultTarget(gameObject).GetComponent<LayoutElement>().DOPreferredSize(to.Value, duration.Value, snapping.Value);
            ...
            tween.Play();
            if (debugThis.Value) State.Debug("DOTween Layout Element Preferred Size");

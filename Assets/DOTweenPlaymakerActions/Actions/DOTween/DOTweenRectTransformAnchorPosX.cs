...
namespace HutongGames.PlayMaker.Actions
{
    ...
    [Tooltip("Tweens the target's anchoredPosition to the given value, tweening only the X axis.")]
    ...
    public class DOTweenRectTransformAnchorPosX : FsmStateAction
    {
        ...
        {
            tween = Fsm.GetOwnerDefaultTarget(gameObject).GetComponent<RectTransform>().DOAnchorPosX(to.Value, duration.Value, snapping.Value);
            ...
            tween.Play();
            if (debugThis.Value) State.Debug("DOTween RectTransform Anchor Pos X");

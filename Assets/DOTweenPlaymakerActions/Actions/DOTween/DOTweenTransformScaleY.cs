...
namespace HutongGames.PlayMaker.Actions
{
    ...
    [Tooltip("Scales the target's localScale to the given value while tweening only the Y axis.")]
    ...
    public class DOTweenTransformScaleY : FsmStateAction
    {
        ...
        [ActionSection("")]
        [Tooltip("Select the target. Either a float or a GameObject's y scale")]
        ...
        public override void OnEnter()
        {
            switch (target)
            {
                case Doozy.PlayMaker.Actions.Target.Value:
                    tween = Fsm.GetOwnerDefaultTarget(gameObject).GetComponent<Transform>().DOScaleY(to.Value, duration.Value);
                    break;
                case Doozy.PlayMaker.Actions.Target.GameObject:
                    tween = Fsm.GetOwnerDefaultTarget(gameObject).GetComponent<Transform>().DOScaleY(toGameObject.Value.transform.localScale.y, duration.Value);
                    break;
            }
            ...
            tween.Play();
            if (debugThis.Value) State.Debug("DOTween Transform Scale Y");

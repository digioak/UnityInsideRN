...
namespace HutongGames.PlayMaker.Actions
{
    ...
    [Tooltip("Fades the target's alpha to the given value (works only with materials that support alpha).")]
    ...
    public class DOTweenMaterialFade : FsmStateAction
    {
        ...
        {
            tween = Fsm.GetOwnerDefaultTarget(gameObject).GetComponent<Renderer>().material.DOFade(to.Value, duration.Value);
            ...
            tween.Play();
            if (debugThis.Value) State.Debug("DOTween Material Fade");

        ...
        public override void OnEnter()
        {
            ...
            switch (target)
            {
                case Doozy.PlayMaker.Actions.Target.Value:
                    sequence.Append(Fsm.GetOwnerDefaultTarget(gameObject).GetComponent<Transform>().DOJump(to.Value, jumpPower.Value, numJumps.Value, duration.Value, snapping.Value).SetRelative(setRelative.Value));
                    break;
                case Doozy.PlayMaker.Actions.Target.GameObject:
                    sequence.Append(Fsm.GetOwnerDefaultTarget(gameObject).GetComponent<Transform>().DOJump(toGameObject.Value.transform.position, jumpPower.Value, numJumps.Value, duration.Value, snapping.Value).SetRelative(setRelative.Value));
                    break;
            }
            ...
            sequence.Play();
            if (debugThis.Value) State.Debug("DOTween Transform Jump");

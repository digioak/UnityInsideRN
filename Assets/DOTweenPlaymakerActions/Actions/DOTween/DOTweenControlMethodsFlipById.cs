...
namespace HutongGames.PlayMaker.Actions
{
    ...
    public class DOTweenControlMethodsFlipById : FsmStateAction
    {
        ...
        public override void OnEnter()
        {
            ...
            switch (tweenIdType)
            {
                case Doozy.PlayMaker.Actions.TweenId.UseString: if (string.IsNullOrEmpty(stringAsId.Value) == false) numberOfTweensFlipped = DOTween.Flip(stringAsId.Value); break;
                case Doozy.PlayMaker.Actions.TweenId.UseTag: if (string.IsNullOrEmpty(tagAsId.Value) == false) numberOfTweensFlipped = DOTween.Flip(tagAsId.Value); break;
                case Doozy.PlayMaker.Actions.TweenId.UseGameObject: if (gameObjectAsId.Value != null) numberOfTweensFlipped = DOTween.Flip(gameObjectAsId.Value); break;
            }
            if (debugThis.Value) State.Debug("DOTween Control Methods Flip By Id - Flipped " + numberOfTweensFlipped + " tweens");
            Finish();
        }
    }
}

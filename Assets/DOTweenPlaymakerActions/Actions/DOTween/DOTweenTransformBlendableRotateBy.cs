...
namespace HutongGames.PlayMaker.Actions
{
    ...
    [Tooltip("Tweens a Transform's rotation BY the given value (as if it was set to relative), in a way that allows other DOBlendableRotate tweens to work together on the same target, instead than fight each other as multiple DORotate would do. NOTE: This is an experimental feature.")]
    ...
    public class DOTweenTransformBlendableRotateBy : FsmStateAction
    {
        ...
        {
            tween = Fsm.GetOwnerDefaultTarget(gameObject).GetComponent<Transform>().DOBlendableRotateBy(by.Value, duration.Value, rotateMode);
            ...
            tween.Play();
            if (debugThis.Value) State.Debug("DOTween Transform Blendable Rotate By");

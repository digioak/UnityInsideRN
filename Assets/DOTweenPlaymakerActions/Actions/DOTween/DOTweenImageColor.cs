...
namespace HutongGames.PlayMaker.Actions
{
    ...
    [Tooltip("Changes the target's color to the given one.")]
    [HelpUrl("http://dotween.demigiant.com/documentation.php")]
    public class DOTweenImageColor : FsmStateAction
    {
        [RequiredField]
        ...
        {
            tween = Fsm.GetOwnerDefaultTarget(gameObject).GetComponent<Image>().DOColor(to.Value, duration.Value);
            ...
            tween.Play();
            if (debugThis.Value) State.Debug("DOTween Image Color");

...
namespace HutongGames.PlayMaker.Actions
{
    ...
    [Tooltip("Changes the light's color to the given one")]
    [HelpUrl("http://dotween.demigiant.com/documentation.php")]
    public class DOTweenLightColor : FsmStateAction
    {
        [RequiredField]
        ...
        {
            tween = Fsm.GetOwnerDefaultTarget(gameObject).GetComponent<Light>().DOColor(to.Value, duration.Value);
            ...
            tween.Play();
            if (debugThis.Value) State.Debug("DOTween Light Color");

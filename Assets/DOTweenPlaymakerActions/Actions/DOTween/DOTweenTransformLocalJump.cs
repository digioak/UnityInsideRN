...
namespace HutongGames.PlayMaker.Actions
{
    ...
    [Tooltip("Tweens the target's localPosition to the given value, while also applying a jump effect along the Y axis. NOTE: Returns a Sequence instead of a Tweener.")]
    [HelpUrl("http://dotween.demigiant.com/documentation.php")]
    public class DOTweenTransformLocalJump : FsmStateAction
    {
        [RequiredField]
        ...
        public override void OnEnter()
        {
            ...
            switch (target)
            {
                case Doozy.PlayMaker.Actions.Target.Value:
                    sequence.Append(Fsm.GetOwnerDefaultTarget(gameObject).GetComponent<Transform>().DOLocalJump(to.Value, jumpPower.Value, numJumps.Value, duration.Value, snapping.Value).SetRelative(setRelative.Value));
                    break;
                case Doozy.PlayMaker.Actions.Target.GameObject:
                    sequence.Append(Fsm.GetOwnerDefaultTarget(gameObject).GetComponent<Transform>().DOLocalJump(toGameObject.Value.transform.position, jumpPower.Value, numJumps.Value, duration.Value, snapping.Value).SetRelative(setRelative.Value));
                    break;
            }
            ...
            sequence.Play();
            if (debugThis.Value) State.Debug("DOTween Transform Local Jump");

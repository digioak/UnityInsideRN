...
namespace HutongGames.PlayMaker.Actions
{
    ...
    [Tooltip("Moves the target's localPosition to the given value, tweening only the Y axis. ")]
    ...
    public class DOTweenTransformLocalMoveY : FsmStateAction
    {
        ...
        [ActionSection("")]
        [Tooltip("Select the target. Either a float or a GameObject's y position")]
        ...
        public override void OnEnter()
        {
            switch (target)
            {
                case Doozy.PlayMaker.Actions.Target.Value:
                    tween = Fsm.GetOwnerDefaultTarget(gameObject).GetComponent<Transform>().DOLocalMoveY(to.Value, duration.Value, snapping.Value);
                    break;
                case Doozy.PlayMaker.Actions.Target.GameObject:
                    tween = Fsm.GetOwnerDefaultTarget(gameObject).GetComponent<Transform>().DOLocalMoveY(toGameObject.Value.transform.position.y, duration.Value, snapping.Value);
                    break;
            }
            ...
            tween.Play();
            if (debugThis.Value) State.Debug("DOTween Transform Local Move Y");

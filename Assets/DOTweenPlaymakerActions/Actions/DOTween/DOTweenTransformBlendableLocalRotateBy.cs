...
namespace HutongGames.PlayMaker.Actions
{
    ...
    [Tooltip("Tweens a Transform's localRotation BY the given value (as if it was set to relative), in a way that allows other DOBlendableRotate tweens to work together on the same target, instead than fight each other as multiple DORotate would do. NOTE: This is an experimental feature.")]
    ...
    public class DOTweenTransformBlendableLocalRotateBy : FsmStateAction
    {
        ...
        {
            tween = Fsm.GetOwnerDefaultTarget(gameObject).GetComponent<Transform>().DOBlendableLocalRotateBy(by.Value, duration.Value, rotateMode);
            ...
            tween.Play();
            if (debugThis.Value) State.Debug("DOTween Transform Blendable Local Rotate By");

...
namespace HutongGames.PlayMaker.Actions
{
    ...
    [Tooltip("Changes the target's color to the given one. Note that this method requires to also insert the start colors for the tween, since LineRenderers have no way to get them.")]
    ...
    public class DOTweenLineRendererColor : FsmStateAction
    {
        ...
        [Tooltip("The start color 1 value to tween from. This method requires the start colors for the tween, since LineRenderers have no way to get them.")]
        public FsmColor startValue_color_1;
        [RequiredField]
        [UIHint(UIHint.FsmColor)]
        [Tooltip("The start color 2 value to tween from. This method requires the start colors for the tween, since LineRenderers have no way to get them.")]
        public FsmColor startValue_color_2;

        [RequiredField]
        [UIHint(UIHint.FsmColor)]
        [Tooltip("The end color 1 value to reach")]
        public FsmColor endValue_color_1;
        [RequiredField]
        [UIHint(UIHint.FsmColor)]
        [Tooltip("The end color 2 value to reach")]
        ...
        public override void Reset()
        {
            base.Reset();
            gameObject = null;
            startValue_color_1 = new FsmColor { UseVariable = false };
            startValue_color_2 = new FsmColor { UseVariable = false };
            endValue_color_1 = new FsmColor { UseVariable = false };
            endValue_color_2 = new FsmColor { UseVariable = false };
        ...
        {
            var startValue = new Color2 { ca = startValue_color_1.Value, cb = startValue_color_2.Value };
            var endValue = new Color2 { ca = endValue_color_1.Value, cb = endValue_color_2.Value };
            tween = Fsm.GetOwnerDefaultTarget(gameObject).GetComponent<LineRenderer>().DOColor(startValue, endValue, duration.Value);
            ...
            tween.Play();
            if (debugThis.Value) State.Debug("DOTween Line Renderer Color");

...
namespace HutongGames.PlayMaker.Actions
{
    ...
    [Tooltip("Tweens a Transform's localPosition BY the given value (as if it was set to relative), in a way that allows other DOBlendableMove tweens to work together on the same target, instead than fight each other as multiple DOMove would do.")]
    [HelpUrl("http://dotween.demigiant.com/documentation.php")]
    public class DOTweenTransformBlendableLocalMoveBy : FsmStateAction
    {
        [RequiredField]
        ...
        public override void Reset()
        {
            base.Reset();
            gameObject = null;
            by = new FsmVector3 { UseVariable = false };
        ...
        {
            tween = Fsm.GetOwnerDefaultTarget(gameObject).GetComponent<Transform>().DOBlendableLocalMoveBy(by.Value, duration.Value, snapping.Value);
            ...
            tween.Play();
            if (debugThis.Value) State.Debug("DOTween Transform Blendable Local Move By");

...
namespace HutongGames.PlayMaker.Actions
{
    ...
    public class DOTweenRigidbodyLookAtPosition : FsmStateAction
    {
        ...
        public override void Reset()
        {
            base.Reset();
            gameObject = null;
            position = new FsmVector3 { UseVariable = false };
        ...
        {
            tween = Fsm.GetOwnerDefaultTarget(gameObject).GetComponent<Rigidbody>().DOLookAt(position.Value, duration.Value, axisConstraint, up.Value);
            ...
            tween.Play();
            if (debugThis.Value) State.Debug("DOTween RigidBody Look At Position");

...
namespace HutongGames.PlayMaker.Actions
{
    ...
    [Tooltip("Tweens the target's text to the given value. ")]
    ...
    public class DOTweenTextText : FsmStateAction
    {
        ...
        [Tooltip("The type of scramble mode to use, if any. If different than ScrambleMode.None the string will appear from a random animation of characters, otherwise it will compose itself regularly. None(default): no scrambling will be applied. All / Uppercase / Lowercase / Numerals: type of characters to be used while scrambling. Custom: will use the custom characters in scrambleChars.")]
        public ScrambleMode scrambleMode;

        [UIHint(UIHint.FsmString)]
        [Tooltip("A string containing the characters to use for custom scrambling. Use as many characters as possible (minimum 10) because DOTween uses a fast scramble mode which gives better results with more characters.")]
        ...
        public override void Reset()
        {
            base.Reset();
            gameObject = null;
            to = new FsmString { UseVariable = false };
            duration = new FsmFloat { UseVariable = false };
            setSpeedBased = new FsmBool { UseVariable = false, Value = false };
            richTextEnabled = new FsmBool { UseVariable = false, Value = true };
            scrambleMode = ScrambleMode.None;
            scrambleChars = new FsmString { UseVariable = false, Value = null };
        ...
        {
            tween = Fsm.GetOwnerDefaultTarget(gameObject).GetComponent<Text>().DOText(to.Value, duration.Value, richTextEnabled.Value, scrambleMode, scrambleChars.Value);
            ...
            tween.Play();
            if (debugThis.Value) State.Debug("DOTween Text Text");

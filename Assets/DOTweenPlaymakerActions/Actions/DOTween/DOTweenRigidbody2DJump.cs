...
namespace HutongGames.PlayMaker.Actions
{
    ...
    public class DOTweenRigidbody2DJump : FsmStateAction
    {
        ...
        [ActionSection("")]
        [Tooltip("Select the target. Either a Vector2 or a GameObject's position")]
        public Doozy.PlayMaker.Actions.Target target = Doozy.PlayMaker.Actions.Target.Value;[UIHint(UIHint.FsmVector2)]
        ...
        public override void OnEnter()
        {
            ...
            switch (target)
            {
                case Doozy.PlayMaker.Actions.Target.Value:
                    sequence.Append(Fsm.GetOwnerDefaultTarget(gameObject).GetComponent<Rigidbody2D>().DOJump(to.Value, jumpPower.Value, numJumps.Value, duration.Value, snapping.Value));
                    break;
                case Doozy.PlayMaker.Actions.Target.GameObject:
                    sequence.Append(Fsm.GetOwnerDefaultTarget(gameObject).GetComponent<Rigidbody2D>().DOJump(new Vector2(toGameObject.Value.transform.position.x, toGameObject.Value.transform.position.y), jumpPower.Value, numJumps.Value, duration.Value, snapping.Value));
                    break;
            }
            ...
            sequence.Play();
            if (debugThis.Value) State.Debug("DOTween RigidBody2D Jump");

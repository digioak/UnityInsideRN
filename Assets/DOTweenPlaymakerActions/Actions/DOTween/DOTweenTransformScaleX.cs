...
namespace HutongGames.PlayMaker.Actions
{
    ...
    [Tooltip("Scales the target's localScale to the given value while tweening only the X axis.")]
    ...
    public class DOTweenTransformScaleX : FsmStateAction
    {
        ...
        [ActionSection("")]
        [Tooltip("Select the target. Either a float or a GameObject's x scale")]
        ...
        public override void OnEnter()
        {
            switch (target)
            {
                case Doozy.PlayMaker.Actions.Target.Value:
                    tween = Fsm.GetOwnerDefaultTarget(gameObject).GetComponent<Transform>().DOScaleX(to.Value, duration.Value);
                    break;
                case Doozy.PlayMaker.Actions.Target.GameObject:
                    tween = Fsm.GetOwnerDefaultTarget(gameObject).GetComponent<Transform>().DOScaleX(toGameObject.Value.transform.localScale.x, duration.Value);
                    break;
            }
            ...
            tween.Play();
            if (debugThis.Value) State.Debug("DOTween Transform Scale X");

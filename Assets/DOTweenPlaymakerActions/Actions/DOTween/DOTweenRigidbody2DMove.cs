...
namespace HutongGames.PlayMaker.Actions
{
    ...
    [Tooltip("Moves the target's position to the given value.")]
        ...
        public override void OnEnter()
        {
            switch (target)
            {
                case Doozy.PlayMaker.Actions.Target.Value:
                    tween = Fsm.GetOwnerDefaultTarget(gameObject).GetComponent<Rigidbody2D>().DOMove(to.Value, duration.Value, snapping.Value);
                    break;
                case Doozy.PlayMaker.Actions.Target.GameObject:
                    tween = Fsm.GetOwnerDefaultTarget(gameObject).GetComponent<Rigidbody2D>().DOMove(toGameObject.Value.transform.position, duration.Value, snapping.Value);
                    break;
            }
            ...
            tween.Play();
            if (debugThis.Value) State.Debug("DOTween RigidBody2D Move");

...
namespace HutongGames.PlayMaker.Actions
{
    ...
    [Tooltip("Shakes a Transform's localPosition with the given values. ")]
    ...
    public class DOTweenTransformShakePosition : FsmStateAction
    {
        ...
        public override void Reset()
        {
            base.Reset();
            gameObject = null;
            strength = new FsmVector3 { UseVariable = false, Value = Vector3.one };
        ...
        {
            tween = Fsm.GetOwnerDefaultTarget(gameObject).GetComponent<Transform>().DOShakePosition(duration.Value, strength.Value, vibrato.Value, randomness.Value, snapping.Value);
            ...
            tween.Play();
            if (debugThis.Value) State.Debug("DOTween Transform Shake Position");

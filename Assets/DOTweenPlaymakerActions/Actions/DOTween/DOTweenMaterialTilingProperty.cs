...
namespace HutongGames.PlayMaker.Actions
{
    ...
    [Tooltip("Changes the target's named textureScale property to the given one.")]
    ...
    public class DOTweenMaterialTilingProperty : FsmStateAction
    {
        ...
        {
            tween = Fsm.GetOwnerDefaultTarget(gameObject).GetComponent<Renderer>().material.DOTiling(to.Value, property.Value, duration.Value);
            ...
            tween.Play();
            if (debugThis.Value) State.Debug("DOTween Material Tiling Property");

...
namespace HutongGames.PlayMaker.Actions
{
    ...
    [Tooltip("Rotates the target so that it will look towards the given GameObject position. ")]
    ...
    public class DOTweenTransformLookAtGameObject : FsmStateAction
    {
        ...
        {
            tween = Fsm.GetOwnerDefaultTarget(gameObject).GetComponent<Transform>().DOLookAt(target.Value.transform.position, duration.Value, axisConstraint, up.Value);
            ...
            tween.Play();
            if (debugThis.Value) State.Debug("DOTween Transform Look At GameObject");

...
namespace HutongGames.PlayMaker.Actions
{
    ...
    [Tooltip("Changes the target's value to the given one.")]
    [HelpUrl("http://dotween.demigiant.com/documentation.php")]
    public class DOTweenSliderValue : FsmStateAction
    {
        [RequiredField]
        ...
        {
            tween = Fsm.GetOwnerDefaultTarget(gameObject).GetComponent<Slider>().DOValue(to.Value, duration.Value, snapping.Value);
            ...
            tween.Play();
            if (debugThis.Value) State.Debug("DOTween Slider Value");

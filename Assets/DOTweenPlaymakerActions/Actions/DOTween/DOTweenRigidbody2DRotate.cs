...
namespace HutongGames.PlayMaker.Actions
{
    ...
    [Tooltip("Rotates the target to the given value.")]
    ...
    public class DOTweenRigidbody2DRotate : FsmStateAction
    {
        ...
        [ActionSection("")]
        [Tooltip("Select the target. Either a Vector2 or a GameObject's rotation")]
        ...
        public override void Reset()
        {
            base.Reset();
            gameObject = null;
            toAngle = new FsmFloat { UseVariable = false };
        ...
        public override void OnEnter()
        {
            switch (target)
            {
                case Doozy.PlayMaker.Actions.Target.Value:
                    tween = Fsm.GetOwnerDefaultTarget(gameObject).GetComponent<Rigidbody2D>().DORotate(toAngle.Value, duration.Value);
                    break;
                case Doozy.PlayMaker.Actions.Target.GameObject:
                    tween = Fsm.GetOwnerDefaultTarget(gameObject).GetComponent<Rigidbody2D>().DORotate(toGameObject.Value.transform.rotation.z, duration.Value);
                    break;
            }
            ...
            tween.Play();
            if (debugThis.Value) State.Debug("DOTween RigidBody2D Rotate");

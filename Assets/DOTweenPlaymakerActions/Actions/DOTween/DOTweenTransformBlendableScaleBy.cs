...
namespace HutongGames.PlayMaker.Actions
{
    ...
    [Tooltip("Tweens a Transform's localScale BY the given value (as if it was set to relative), in a way that allows other DOBlendableScale tweens to work together on the same target, instead than fight each other as multiple DOScale would do.")]
    ...
    public class DOTweenTransformBlendableScaleBy : FsmStateAction
    {
        ...
        {
            tween = Fsm.GetOwnerDefaultTarget(gameObject).GetComponent<Transform>().DOBlendableScaleBy(by.Value, duration.Value);
            ...
            tween.Play();
            if (debugThis.Value) State.Debug("DOTween Transform Blendable Scale By");
